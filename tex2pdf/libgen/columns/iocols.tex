...
\newdimen\colprevdepth
\newdimen\colthisht
\def\beginiocolumnns{\begingroup\widowpenalty=10000 \colprevdepth=0pt
...
\long\def\colout#1\colin#2\colpar{%
	\prevdepth=-\maxdimen
	\begingroup%
	\computefinalheightfrom{#1}%
	\storecol{#1}{\outcolumn}%
	\storecol{#2}{\incolumn}%
	\updatefreespace%
	\buildpagesofar%
	\insertinterrowskip%
	\global\colprevdepth=\dp0%
	\box0%
	\buildnextpage%
	\endgroup%
}
...
\def\insertinterrowskip{\ifdim\pagetotal>\textheight \insertskipfalse\fi
	\ifinsertskip%
		\begingroup%
		\setbox0=\hbox to \textwidth{\hfil\vrule depth5mm\hfil}%
		\ht0=0pt \dp0=0pt \box0 \prevdepth=-\maxdimen%
		\endgroup%
		\vskip0pt plus 0.5pt\fi}

\def\buildpagesofar{\splittopskip=\topskip \splitmaxdepth=2pt%
	\dimen0=\freespace%
	\ifdim\freespace>\baselineskip
		\setbox0=\vsplit\outcolumn upto \dimen0%
		\setbox2=\vsplit\incolumn upto \dimen0%
		\configurepage%
	\else\global\insertskipfalse\setbox0=\box\voidbox\fi
}
...
\def\buildnextpage{\global\insertskipfalse\ifvoidiocols
		\global\insertskiptrue\penalty0\else
...
\def\computefinalheightfrom#1{%
	\setbox0=\vtop{#1}%
	\global\colthisht=\ht0}

\def\computeinterlineskip{\dimen0=\baselineskip%
	\advance\dimen0 by -\colprevdepth%
	\advance\dimen0 by -\colthisht%
	\ifdim\dimen0<0pt\dimen0=\lineskip\fi}
